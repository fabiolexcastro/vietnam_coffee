...
# Load data ---------------------------------------------------------------
prd <- '2021_2040'
fls <- dir_ls(glue('../1.Data/climate/future/cmip6/ssp370/2021_2040/wc2.1_30s'), regexp = '.tif$')
...
fls %>% 
  grep('bio_1_', ., value = TRUE) %>% 
  as.character()

# Create normal bioclimatic -----------------------------------------------
map(.x = 9:length(mdl), .f = function(k){
...
bios <- glue('bio_{1:19}_')
dir_ls('../1.Data/climate/future/cmip6/ssp370/2021_2040/wc2.1_30s', regexp = '.tif$') %>% 
...
map(.x = 4:length(mdl), .f = function(k){
  ...
  cat('To calculate other biocklimatic variables\n') 
